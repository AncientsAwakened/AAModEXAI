...
namespace AAModEXAI.Bosses.Equinox
{
	...
	{	
		...
		public override void SetDefaults()
		{
            ...
            music = ModLoader.GetMod("AAMod").GetSoundSlot(SoundType.Music, "Sounds/Music/Equinox");
        ...
        public override bool PreAI()
        {
            ...
            if (wormStronger)
            {
                ...
                npc.damage = 300;
				npc.defense = ((!this.nightcrawler) ? 120 : 150);
            }
                ...
                if(isHead && npc.type == mod.NPCType("DaybringerHead"))
                {
                        ...
                        for(int playerid = 0; playerid < 255; playerid++)
                        {
                            if(Main.player[playerid].active && !Main.player[playerid].dead && Main.player[playerid] != null && Main.player[playerid].ownedProjectileCounts[mod.ProjectileType("DaybringerStars")] <= 0)
                            {
                                if (npc.life > npc.lifeMax / 2)
                                {
                                    ...
                                    {
                                        Projectile.NewProjectile(Main.player[playerid].Center.X - 200f, Main.player[playerid].Center.Y + 200f, 0, 0, mod.ProjectileType("DaybringerStars"), damage/ 3, 5, playerid, -200f, playerid);
                                        Projectile.NewProjectile(Main.player[playerid].Center.X, Main.player[playerid].Center.Y - 300f, 0, 0, mod.ProjectileType("DaybringerStars"), damage/ 3, 5, playerid, 0, playerid);
                                        Projectile.NewProjectile(Main.player[playerid].Center.X + 200f, Main.player[playerid].Center.Y + 200f, 0, 0, mod.ProjectileType("DaybringerStars"), damage/ 3, 5, playerid, 200f, playerid);
                                    }
                                    else
                                    {
                                ...
                                else
                                {
                                    ...
                                    else
                                    {
                                        Projectile.NewProjectile(Main.player[playerid].Center.X, Main.player[playerid].Center.Y + 300f, 0, 0, mod.ProjectileType("DaybringerStars"), damage/ 3, 5, playerid, 0, playerid);
                                        Projectile.NewProjectile(Main.player[playerid].Center.X, Main.player[playerid].Center.Y - 300f, 0, 0, mod.ProjectileType("DaybringerStars"), damage/ 3, 5, playerid, 0, playerid);
                                        Projectile.NewProjectile(Main.player[playerid].Center.X + 300f, Main.player[playerid].Center.Y, 0, 0, mod.ProjectileType("DaybringerStars"), damage/ 3, 5, playerid, 0, playerid);
                                        Projectile.NewProjectile(Main.player[playerid].Center.X - 300f, Main.player[playerid].Center.Y, 0, 0, mod.ProjectileType("DaybringerStars"), damage/ 3, 5, playerid, 0, playerid);
                                    }
                                }
                            }
                    ...
                    if(internalAI[0] % 120 == 30 && Main.netMode != NetmodeID.MultiplayerClient)
                    {
                        for (int i = 0; i < Main.maxNPCs; i += 1)
                        {
                            if (Main.npc[i].active && Main.npc[i].type == mod.NPCType("DaybringerBody") && Main.npc[i].realLife == npc.whoAmI)
                            {
                                ...
                                Projectile.NewProjectile(Main.npc[i].Center.X, Main.npc[i].Center.Y, speed.X, speed.Y, mod.ProjectileType("DayBringerDarts"), damage/ 3, 0, Main.myPlayer);
                                speed = -speed;
                                Projectile.NewProjectile(Main.npc[i].Center.X, Main.npc[i].Center.Y, speed.X, speed.Y, mod.ProjectileType("DayBringerDarts"), damage/ 3, 0, Main.myPlayer);
                            }
                        }
                    ...
                    if(internalAI[0] % 120 == 60 && Main.netMode != NetmodeID.MultiplayerClient)
                    {
                        for (int i = 0; i < Main.maxNPCs; i+=2)
                        {
                            ...
                            {
                                Vector2 speed = Vector2.Normalize(new Vector2(1f, 0f).RotatedBy(Main.npc[i].rotation + 3.1415f)) * 8f;
                                Projectile.NewProjectile(Main.npc[i].Center.X, Main.npc[i].Center.Y, speed.X, speed.Y, mod.ProjectileType("DaybringerOrb"), damage/ 3, 0, Main.myPlayer, 0, npc.whoAmI);
                                speed = -speed;
                                Projectile.NewProjectile(Main.npc[i].Center.X, Main.npc[i].Center.Y, speed.X, speed.Y, mod.ProjectileType("DaybringerOrb"), damage/ 3, 0, Main.myPlayer, 0, npc.whoAmI);
                            }
                        }
                ...
                if(isHead && npc.type == mod.NPCType("NightcrawlerHead"))
                {
                    ...
                    if(internalAI[1] % 150 == 0 && Main.netMode != NetmodeID.MultiplayerClient)
                    {
                        for(int playerid = 0; playerid < 255; playerid++)
                        {
                            ...
                            {
                                Projectile.NewProjectile(Main.player[playerid].Center.X + Main.rand.Next(-5, 5) * 40f, Main.player[playerid].Center.Y + Main.rand.Next(-5, 5) * 40f, 0, 0, mod.ProjectileType("NightclawerCloud"), damage/ 6, 0, 255);
                            }
                        }
                    }
                    ...
                    if(internalAI[1] % 380 == 90 && Main.netMode != NetmodeID.MultiplayerClient)
                    {
                        for (int i = 0; i < Main.maxNPCs; i+= 1)
                        {
                            if (Main.npc[i].active && Main.npc[i].type == mod.NPCType("NightcrawlerBody") && Main.npc[i].realLife == npc.whoAmI)
                            {
                                ...
                                Projectile.NewProjectile(Main.npc[i].Center.X, Main.npc[i].Center.Y, speed.X, speed.Y, mod.ProjectileType("NightclawerScythe"), damage/ 3, 0, Main.myPlayer, npc.rotation, npc.spriteDirection);
                                speed = -speed;
                                Projectile.NewProjectile(Main.npc[i].Center.X, Main.npc[i].Center.Y, speed.X, speed.Y, mod.ProjectileType("NightclawerScythe"), damage/ 3, 0, Main.myPlayer, npc.rotation, npc.spriteDirection);
                            }
                        }
                    ...
                    if(internalAI[1] % 120 == 90 && Main.netMode != NetmodeID.MultiplayerClient)
                    {
                        ...
                        {
                            if (Main.npc[i].active && Main.npc[i].type == mod.NPCType("NightcrawlerBody") && Main.npc[i].realLife == npc.whoAmI && Main.rand.Next(4) == 0)
                            {
                                ...
                                Projectile.NewProjectile(Main.npc[i].Center.X, Main.npc[i].Center.Y, speedR.X, speedR.Y, mod.ProjectileType("NightclawerLaser"), damage/ 3, 0, Main.myPlayer, speed.ToRotation() + 1000f, ai);
                            }
                        }
                    }
        ...
        public override void NPCLoot()
        {
			...
			{
				Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModLoader.GetMod("AAMod").ItemType(wormType + "Trophy"));
			}
			if (Main.expertMode)
			{
			...
			else
			{
				...
				{
					Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModLoader.GetMod("AAMod").ItemType(wormType + "Mask"));
				}
                if (!nightcrawler)
                {
                    Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModLoader.GetMod("AAMod").ItemType("Stardust"), Main.rand.Next(30, 75));
                }
                else
                {
                    Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModLoader.GetMod("AAMod").ItemType("DarkEnergy"), Main.rand.Next(30, 75));
                }
                if (AAWorld.RadiumOre)
                {
                    Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModLoader.GetMod("AAMod").ItemType("StarIdol"));
                }
			}
        }
