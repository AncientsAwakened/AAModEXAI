...
namespace AAModEXAI.Bosses.Sag
{
    ...
    public class Sag : ModNPC
	{
		...
		public override void SetDefaults()
        {
            ...
            music = ModLoader.GetMod("AAMod").GetSoundSlot(SoundType.Music, "Sounds/Music/Sag");
            npc.value = 80000f;
            npc.knockBackResist = 0f;
            npc.noGravity = true;
            npc.noTileCollide = true;
            bossBag = ModLoader.GetMod("AAMod").ItemType("SagBag");
        }
        ...
        public void Shooting()
        {
            ...
            switch ((int)npc.ai[3])
            {
                ...
                case 1:
                    ...
                    if (internalAI[0] > 80)
                    {
                        BaseAI.ShootPeriodic(npc, player.Center + new Vector2(Main.rand.Next(-10, 10), Main.rand.Next(-10, 10)), player.width, player.height, mod.ProjectileType("SagiStar"), ref npc.ai[2], 20, npc.damage / 4, 9, false, new Vector2(36 * npc.direction, -51));
                    }
                    break;
                case 2:
                    if (Main.netMode != NetmodeID.MultiplayerClient)
                ...
                case 3:
                    ...
                    if (internalAI[0] > 80)
                    {
                        BaseAI.ShootPeriodic(npc, player.Center + new Vector2(Main.rand.Next(-10, 10), Main.rand.Next(-10, 10)), player.width, player.height, mod.ProjectileType("SagRocket"), ref npc.ai[2], 40, npc.damage / 4, 9, false);
                    }
                    break;
                case 4:
                    if (Main.netMode != NetmodeID.MultiplayerClient)
                    ...
                    if (internalAI[0] > 80)
                    {
                        BaseAI.ShootPeriodic(npc, player.Center + new Vector2(Main.rand.Next(-10, 10), Main.rand.Next(-10, 10)), player.width, player.height, mod.ProjectileType("SagBlast"), ref npc.ai[2], 50, npc.damage / 4, 9, false, new Vector2(36 * npc.direction, -51));
                    }
                    break;
                default:
                    npc.ai[3] = 0;
        ...
        public override bool PreDraw(SpriteBatch sb, Color dColor)
        {
            ...
            BaseDrawing.DrawTexture(sb, ModLoader.GetMod("AAMod").GetTexture("Glowmasks/Sag_Glow"), 0, npc.position, npc.width, npc.height, npc.scale, npc.rotation, npc.direction, 9, npc.frame, npc.GetAlpha(ColorUtils.COLOR_GLOWPULSE), false);
            return false;
        }
        ...
        public override void NPCLoot()
        {
            ...
            {
                Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModLoader.GetMod("AAMod").ItemType("SagTrophy"));
            }
            if (!Main.expertMode)
            {
                if (Main.rand.Next(7) == 0)
                {
                    npc.DropLoot(ModLoader.GetMod("AAMod").ItemType("SagMask"));
                }
                string[] lootTable = { "SagCore", "NeutronStaff", "Legg" };
                int loot = Main.rand.Next(lootTable.Length);
                npc.DropLoot(ModLoader.GetMod("AAMod").ItemType(lootTable[loot]));
                Item.NewItem(npc.Center, ModLoader.GetMod("AAMod").ItemType("Doomite"), Main.rand.Next(20, 30));
            }
            else
            {
