...
namespace AAModEXAI.Bosses.Yamata
{
    public class YamataTransition : ModNPC
    {
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
        {
            ...
            BaseDrawing.DrawTexture(spriteBatch, ModLoader.GetMod("AAMod").GetTexture("Glowmasks/YamataTransition"), 0, npc.position, npc.width, npc.height, npc.scale, npc.rotation, npc.direction, 24, npc.frame, npc.GetAlpha(new Color(RVal, 0, BVal)), true);
            return false;
        }
        ...
        public override void AI()
        {
			...
			if(Main.netMode != 1)
			{
				...
				if (npc.ai[0] >= 1455 && !NPC.AnyNPCs(mod.NPCType("YamataA")))
				{
                    ...
                    {
                        int num88 = Dust.NewDust(position, num84, height3, ModLoader.GetMod("AAMod").DustType("YamataADust"), 0f, 0f, 200, default, 3.7f);
                        Main.dust[num88].position = npc.Center + (Vector2.UnitY.RotatedByRandom(3.1415927410125732) * (float)Main.rand.NextDouble() * num84 / 2f);
                        Main.dust[num88].noGravity = true;
                        Main.dust[num88].noLight = true;
                        Main.dust[num88].velocity *= 3f;
                        Main.dust[num88].velocity += npc.DirectionTo(Main.dust[num88].position) * (2f + (Main.rand.NextFloat() * 4f));
                        num88 = Dust.NewDust(position, num84, height3, ModLoader.GetMod("AAMod").DustType("YamataADust"), 0f, 0f, 100, default, 1.5f);
                        Main.dust[num88].position = npc.Center + (Vector2.UnitY.RotatedByRandom(3.1415927410125732) * (float)Main.rand.NextDouble() * num84 / 2f);
                    ...
                    {
                        int num90 = Dust.NewDust(position, num84, height3, ModLoader.GetMod("AAMod").DustType("YamataADust"), 0f, 0f, 0, default, 2.7f);
                    ...
                    {
                        int num92 = Dust.NewDust(position, num84, height3, ModLoader.GetMod("AAMod").DustType("YamataADust"), 0f, 0f, 0, default, 1.5f);
