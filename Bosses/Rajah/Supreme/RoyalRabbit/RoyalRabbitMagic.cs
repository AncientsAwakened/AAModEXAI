                ...
                if(npc.ai[0] ++ > 15)
                {
                    if(Main.netMode != NetmodeID.MultiplayerClient)
                    {
                        ...
                        for (int i = 0; i < 3; i++)
                        {
                            ...
                            int proj = Projectile.NewProjectile(npc.Center.X, npc.Center.Y, baseSpeed*(float)Math.Sin(offsetAngle), baseSpeed*(float)Math.Cos(offsetAngle), mod.ProjectileType("CarrowR"), npc.damage / 2, 5, Main.myPlayer);
                        }
                    }
