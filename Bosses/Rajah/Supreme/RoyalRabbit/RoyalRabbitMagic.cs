...
{
    public class RoyalRabbitMagic : ModNPC
    {
        public override void SetStaticDefaults()
        {
            DisplayName.SetDefault("Royal Rabbit Mage");
            ...
            if(npc.Center.Y > player.Center.Y - 100f && npc.Center.Y < player.Center.Y + 100f)
            {
                if(npc.ai[0] ++ > 15)
                {
                    if(Main.netMode != NetmodeID.MultiplayerClient)
                    {
                        ...
                        float speedX = 11f * shootdirection;
                        float speedY = 0;
                        ...
                        for (int i = 0; i < 3; i++)
                        {
                            ...
                            int proj = Projectile.NewProjectile(npc.Center.X, npc.Center.Y, baseSpeed*(float)Math.Sin(offsetAngle), baseSpeed*(float)Math.Cos(offsetAngle), ModContent.ProjectileType<CarrowR>(), npc.damage / 2, 5, Main.myPlayer);
                        }
                    }
