...
namespace AAModEXAI.Bosses.Zero
{
    ...
    public class RealityCannon : ModNPC
    {
            ...
            if (npc.ai[2] == aiTimerFire)
            {
                ...
                if (Collision.CanHit(npc.position, npc.width, npc.height, player.Center, player.width, player.height))
                {
                    ...
                    for (int i = 0; i < (Main.expertMode ? 3 : 4); i++)
                    {
                        ...
                        Projectile.NewProjectile(npc.Center.X, npc.Center.Y, baseSpeed * (float)Math.Sin(offsetAngle), baseSpeed * (float)Math.Cos(offsetAngle), mod.ProjectileType("DeathLaser"), (int)(npc.damage / 1.5f), 5, Main.myPlayer);
                    }
                }
            }
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Color drawColor)
        {
            ...
            Texture2D glowTex = ModLoader.GetMod("AAMod").GetTexture("Glowmasks/RealityCannonZ");
