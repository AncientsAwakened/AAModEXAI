...
namespace AAModEXAI.Base.AAStuff
{
    public class AAAI
	{
        ...
        public static void AIClaw (NPC npc, ref float[] ai, bool isDragonClaw = true, bool ignoreWet = false, float moveIntervalX = 0.1f, float moveIntervalY = 0.04f, float velMaxX = 4f, float velMaxY = 1.5f, float bounceScalarX = 1f, float bounceScalarY = 1f)
        {
            ...
            bool ZoneInferno = (bool) ModSupport.GetModPlayerConditions("AAMod", Main.player[npc.target], "AAPlayer", "ZoneInferno", false, false);
            if (((isDragonClaw && ZoneInferno && Main.dayTime) || (!isDragonClaw && Main.dayTime)) && npc.position.Y <= Main.worldSurface * 16.0)
            {
