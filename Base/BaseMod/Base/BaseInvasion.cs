namespace AAModEXAI
{
    /*public class BaseInvasion
	{
		...
		public void Setup()
		{
			...
			zombieNet = new int[] { -45, -44, -37, -36, -35, -34, -33, -32, -31, -30, -29, -28, -27, -26, 3, 132, 186, 187, 188, 189, 200, ModContent.NPCType<TorchZombie>(), ModContent.NPCType<FitnessZombie>(), ModContent.NPCType<MaggotZombie>() };
			skeletonNet = new int[] { -46, -47, -48, -49, -50, -51, -52, -53, 21 };
			zombieHalloweenNet = new int[] { 319, 320, 321, 316 };
			zombieHMNet = new int[] { ModContent.NPCType<Undead1a>(), ModContent.NPCType<Undead2a>(), ModContent.NPCType<Undead3a>(), ModContent.NPCType<Undead4a>(), ModContent.NPCType<Undead1b>(), ModContent.NPCType<Undead2b>(), ModContent.NPCType<Undead3b>(), ModContent.NPCType<Undead4b>(), ModContent.NPCType<Undead1c>(), ModContent.NPCType<Undead2c>(), ModContent.NPCType<Undead3c>(), ModContent.NPCType<Undead4c>() }; //undead
		...
		{
			Mod mod = GRealm.self;
			if(type == ModContent.NPCType<Barbarian>() || type == ModContent.NPCType<UndeadipedeHead>() || type == ModContent.NPCType<BumblebirbRider>()) return true;
		...
		{
			Mod mod = GRealm.self;			
			if(type == ModContent.NPCType<Barbarian>()){ return "A Barbarian roars into battle!"; }
			if(type == ModContent.NPCType<UndeadipedeHead>()){ return "An Undeadipede hungers for flesh..."; }
			if(type == ModContent.NPCType<BumblebirbRider>()){ return "A Bumblebirb Rider buzzes along..."; }
		...
		{
			Mod mod = GRealm.self;			
			if(type == ModContent.NPCType<Barbarian>()){ return "A Barbarian has perished!"; }		
			if(type == ModContent.NPCType<UndeadipedeHead>()){ return "An Undeadipede withers away..."; }
			if(type == ModContent.NPCType<BumblebirbRider>()){ return "A Bumblebirb Rider has been fried!"; }		
		...
		public static IDictionary<int, float> SpawnPool(IDictionary<int, float> startingPool, int x, int y, Player player)
		{
			...
			if (Main.hardMode) //hardmode zombies
			{
				if(spawnHardMobCount > 0 && hardMobs < hardMobsMax && hardMobsCooldown == 0)
				{
					...
					{
						addedHardMob = true;
						net.Add(ModContent.NPCType<BumblebirbRider>());				
					}else
					...
					{
						addedHardMob = true;
						net.Add(ModContent.NPCType<Barbarian>());
					}
				...
				{
					net.Add(ModContent.NPCType<Crawler>());
				}
				if (ZONEDEV || player.InZone("Desert")) //hollow husk
				{
					net.Add(ModContent.NPCType<HollowHusk>());
				}
				...
				if ((DEV || NPC.downedBoss1) && Main.rand.Next(rarityHM) == 0) //eyemaster, gundead, dynadead
				{
					...
					{
						net.Add(ModContent.NPCType<BlightedEyemaster>());
					}else
					{
						net.Add(ModContent.NPCType<Eyemaster>());
					}
					if(!NPC.downedMoonlord || Main.rand.Next(3) == 0) net.Add(ModContent.NPCType<Gundead>());
				}
			...
			}else //normal zombies
			{
				...
				{
					net.Add(ModContent.NPCType<SandsweptZombie>());
				}

				#region boss specials
				if ((DEV || NPC.downedBoss1) && Main.rand.Next(rarity) == 0) //eyerider, bowman, grenader zombie
				{
					net.Add(ModContent.NPCType<Eyerider>());
					net.Add(ModContent.NPCType<BowmanZombie>());
					net.Add(ModContent.NPCType<GrenaderZombie>());
				}
				if ((DEV || NPC.downedBoss2) && Main.rand.Next(rarity + 1) == 0) //crude caster, corrupt caster, crim caster, cold caster
				{
					net.Add(ModContent.NPCType<CrudeCaster>());
					if (ZONEDEV || player.InZone("Corruption"))
					{
						net.Add(ModContent.NPCType<CorruptCaster>());
					}
					if (ZONEDEV || player.InZone("Crimson"))
					{
						net.Add(ModContent.NPCType<CrimCaster>());
					}
					if (ZONEDEV || player.InZone("Snow"))
					{
						net.Add(ModContent.NPCType<ColdCaster>());
					}
				}
				if ((DEV || NPC.downedBoss3) && Main.rand.Next(rarity) == 0) //librarapprentice, bone warrior
				{
					net.Add(ModContent.NPCType<BoneWarrior>());
				}
				if ((DEV || NPC.downedQueenBee) && (ZONEDEV || player.InZone("Jungle")) && Main.rand.Next(rarity) == 0) //beekeeper (2/5)
				{
					net.Add(ModContent.NPCType<Beekeeper>());
				}
				#endregion
				...
				{
					net.Add(ModContent.NPCType<FloatyZombie>());
				}
				if (ZONEDEV || player.ZoneBeach) //scuba zombie
				{
					net.Add(ModContent.NPCType<ScubaZombie>());
				}				
			}
