...
namespace AAModEXAI
{
    public class BaseUseStyle
    {
		 ...
		 * 
		 * ProjectileType : The type of the projectile to be aiming at.
		 * isIndex : If true, takes ProjectileType as the index in the projectile array instead of as a type.
		 */

		public static void SetStyleHarpoon(Player player, Item item, int ProjectileType, bool isIndex = false)
		{
            int projID = isIndex ? ProjectileType : BaseAI.GetProjectile(player.Center, ProjectileType, player.whoAmI, 0f);
			if(projID != -1)
			{
