...
namespace AAModEXAI
{
    public class BaseAI
    {
		...
		public static void AIProjSpaceOctopus(Projectile p, ref float[] ai, int parentNPCType, int fireProjectileType = -1, float shootVelocity = 16f, float hoverTime = 210f, float xMult = 0.15f, float yMult = 0.075f, Action<int, Projectile> SpawnDust = null)
		{
			AIProjSpaceOctopus(p, ref ai, parentNPCType, fireProjectileType, shootVelocity, hoverTime, xMult, yMult, -1, true, false, SpawnDust);
		}

		public static void AIProjSpaceOctopus(Projectile projectile, ref float[] ai, int parentNPCType, int fireProjectileType = -1, float shootVelocity = 16f, float hoverTime = 210f, float xMult = 0.15f, float yMult = 0.075f, int fireDmg = -1, bool useParentTarget = true, bool noParentHover = false, Action<int, Projectile> SpawnDust = null)
		{
			...
			if (ai[0] == hoverTime)
			{
				...
				}else
				{
					...
					{
						Projectile.NewProjectile(projectile.Center.X, projectile.Center.Y, distVecNormal.X * shootVelocity, distVecNormal.Y * shootVelocity, fireProjectileType, fireDmg, 1f, Main.myPlayer, 0f, 0f);
					}
					ai[0] = 0f;
				}
			}
		...
		*/
		...
		public static int GetProjectile(Vector2 center, int ProjectileType = -1, int owner = -1, float distance = -1, Func<Projectile, bool> CanAdd = null)
		{
			return GetProjectile(center, ProjectileType, owner, default, distance, CanAdd);
		}
		/*
		 ...
		 * 
		 * ProjectileType : If -1, check for ANY projectiles in the area. If not, check for the projectiles who match the type given.
		 * projsToExclude : An array of projectile whoAmIs to exclude from the search.
		 * distance : The distance to check.
		 */
		public static int GetProjectile(Vector2 center, int ProjectileType = -1, int owner = -1, int[] projsToExclude = default, float distance = -1, Func<Projectile, bool> CanAdd = null)
		{
			int currentProj = -1;
			for (int i = 0; i < Main.projectile.Length; i++)
			{
				Projectile proj = Main.projectile[i];
				if (proj != null && proj.active && (ProjectileType == -1 || proj.type == ProjectileType) && (owner == -1f || proj.owner == owner) && (distance == -1f || proj.Distance(center) < distance))
				{
					bool add = true;
		...
		public static int[] GetProjectiles(Vector2 center, int ProjectileType = -1, int owner = -1, float distance = 500f, Func<Projectile, bool> CanAdd = null)
		{
			return GetProjectiles(center, ProjectileType, owner, default, distance, CanAdd);
		}
		/*
         ...
         * distance : The distance to check.
		 */
		public static int[] GetProjectiles(Vector2 center, int ProjectileType = -1, int owner = -1, int[] projsToExclude = default, float distance = 500f, Func<Projectile, bool> CanAdd = null)
		{
			List<int> allProjs = new List<int>();
			for (int i = 0; i < Main.projectile.Length; i++)
			{
				Projectile proj = Main.projectile[i];
				if (proj != null && proj.active && (ProjectileType == -1 || proj.type == ProjectileType) && (owner == -1 || proj.owner == owner) && (distance == -1 || proj.Distance(center) < distance))
				{
					bool add = true;
					if (projsToExclude != default)
					{
		...
		public static int[] GetProjectiles(Vector2 center, int[] ProjectileTypes, int owner = -1, float distance = 500f, Func<Projectile, bool> CanAdd = null)
		{
			return GetProjectiles(center, ProjectileTypes, owner, default, distance, CanAdd);
		}
         ...
         * distance : The distance to check.
		 */
		public static int[] GetProjectiles(Vector2 center, int[] ProjectileTypes, int owner = -1, int[] projsToExclude = default, float distance = 500f, Func<Projectile, bool> CanAdd = null)
		{
			List<int> allProjs = new List<int>();
			for (int i = 0; i < Main.projectile.Length; i++)
			{
				...
				{
					bool isType = false;
					foreach (int type in ProjectileTypes) { if (proj.type == type) { isType = true; break; } }
					if (!isType) { continue; }
					bool add = true;
					if (projsToExclude != default)
					{
         ...
         */
        public static int ShootPeriodic(Entity codable, Vector2 position, int width, int height, int ProjectileType, ref float delayTimer, float delayTimerMax = 100f, int damage = -1, float speed = 10f, bool checkCanHit = true, Vector2 offset = default)
        {
            int pID = -1;
            if (damage == -1) { Projectile proj = new Projectile(); proj.SetDefaults(ProjectileType); damage = proj.damage; }
            ...
            if (properSide)
            {
                ...
                if (delayTimer <= 0)
                {
                    if (!checkCanHit || Collision.CanHit(codable.position, codable.width, codable.height, position, width, height))
                    {
						...
						pID = FireProjectile(targetCenter, fireTarget, ProjectileType, damage, 0f, speed);
                    }
